...
InjuryProcessor::createParameterLayout()
{
...
InjuryProcessor::InjuryProcessor()
    : AudioProcessor (BusesProperties()
      ...
      apvts (*this, nullptr, "Injury", createParameterLayout())
{
    // ADSR + global
...
//==============================================================================
void InjuryProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
...
void InjuryProcessor::releaseResources() {}

//==============================================================================
void InjuryProcessor::processBlock (juce::AudioBuffer<float>& buffer,
                                     juce::MidiBuffer& midiMessages)
{
    juce::ScopedNoDenormals noDenormals;

    // Guard: FL Studio's Fruity Wrapper can call processBlock with a 0-channel
    // buffer during initialisation — bail out safely to avoid access violations.
...
//==============================================================================
void InjuryProcessor::applyFxParams()
{
...
//==============================================================================
bool InjuryProcessor::loadSampleFile (int oscIndex, const juce::File& file)
{
...
void InjuryProcessor::setWavetablePosition (int oscIndex, float pos)
{
...
float InjuryProcessor::getWavetablePosition (int oscIndex) const
{
    if (oscIndex < 0 || oscIndex >= NUM_OSC) return 0.5f;
    return oscStates[(size_t)oscIndex].wavetablePosition.load();
}

void InjuryProcessor::rebuildWavetableFromPosition (int oscIndex, float pos)
{
    auto& state = oscStates[(size_t)oscIndex];
...
//==============================================================================
void InjuryProcessor::startAnalysis (int oscIndex)
{
    if (oscIndex < 0 || oscIndex >= NUM_OSC) return;
    auto& state = oscStates[(size_t)oscIndex];
...
void InjuryProcessor::cancelAnalysis (int oscIndex)
{
    if (oscIndex >= 0 && oscIndex < NUM_OSC)
        oscStates[(size_t)oscIndex].analysisCancel.store (true);
}

bool InjuryProcessor::isAnalyzing (int oscIndex) const
{
    if (oscIndex < 0 || oscIndex >= NUM_OSC) return false;
    return oscStates[(size_t)oscIndex].analyzing.load();
}

bool InjuryProcessor::hasSampleLoaded (int oscIndex) const
{
    if (oscIndex < 0 || oscIndex >= NUM_OSC) return false;
    return oscStates[(size_t)oscIndex].sampleLoaded.load();
}

const juce::AudioBuffer<float>& InjuryProcessor::getSampleBuffer (int oscIndex) const
{
    return oscStates[(size_t)oscIndex].sampleBuffer;
}

void InjuryProcessor::onAnalysisComplete (int oscIndex, float bestPos)
{
...
//==============================================================================
void InjuryProcessor::parameterChanged (const juce::String& paramID, float newValue)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        if (paramID == "wtPosition" + idx)
        {
            ...
            if (std::abs (newValue - current) > 0.0005f)
            {
                ...
                rebuildWavetableFromPosition (i, newValue);
            }
            return;
        }
...
void InjuryProcessor::setStateInformation (const void* data, int sizeInBytes)
{
