...
//==============================================================================
InjuryEditor::InjuryEditor (InjuryProcessor& p)
    : AudioProcessorEditor (&p), proc (p), adsrViz (p)
{
    ...
    for (int i = 0; i < NUM_OSC; i++)
    {
        ...
        pnl->titleLabel.setColour (juce::Label::textColourId, InjuryLAF::kAccent);
        ...
        // Rotary knobs — vol, pan, pitch, detune, phase, unison
    ...
    statusLabel.setColour (juce::Label::textColourId, InjuryLAF::kDim);
...
//==============================================================================
void InjuryEditor::makeRotary (juce::Slider& s)
{
...
void InjuryEditor::makeLabel (juce::Label& l, const juce::String& text)
{
    l.setText (text, juce::dontSendNotification);
    l.setJustificationType (juce::Justification::centred);
    l.setFont (juce::Font (8.5f, juce::Font::bold));
    l.setColour (juce::Label::textColourId, InjuryLAF::kDim);
    addAndMakeVisible (l);
}

//==============================================================================
void InjuryEditor::buildFxModules()
{
    // HyperDimension
...
InjuryEditor::FxModule* InjuryEditor::buildFxModule (
    ...
    const juce::StringArray& knobNames)
{
    ...
    m->nameLabel.setColour (juce::Label::textColourId, InjuryLAF::kAccent.withAlpha (0.9f));
...
//==============================================================================
void InjuryEditor::paint (juce::Graphics& g)
{
    // Solid black base
    g.fillAll (InjuryLAF::kBg);

    // Full-bleed background image (dimmed for readability)
    static const juce::Image bgImage = juce::ImageCache::getFromMemory (
    ...
    // Title in top-left
    InjuryLAF::paintInjuryTitle (g, juce::Rectangle<int> (16, 6, 320, 44));

    // Subtle header underline
    g.setColour (InjuryLAF::kAccent.withAlpha (0.55f));
    g.drawHorizontalLine (50, 0.0f, (float)kW);

    if (currentTab == 0)
    {
        // Oscillator section bg
        InjuryLAF::paintGrungePanelBg (g, juce::Rectangle<int> (8, 54, kW - 16, 310), "OSCILLATORS");

        // Envelope section bg
        InjuryLAF::paintGrungePanelBg (g, juce::Rectangle<int> (8, 372, 380, 298), "ENVELOPE");

        // Global section bg
        InjuryLAF::paintGrungePanelBg (g, juce::Rectangle<int> (396, 372, kW - 404, 298), "GLOBAL");
        ...
        for (int i = 0; i < NUM_OSC; i++)
        {
            ...
            g.setColour (InjuryLAF::kAccent.withAlpha (0.9f));
    ...
    {
        // FX tab background
        InjuryLAF::paintGrungePanelBg (g, juce::Rectangle<int> (8, 54, kW - 16, kH - 62), "FX CHAIN");
    }
}

//==============================================================================
void InjuryEditor::resized()
{
    // ── Header ────────────────────────────────────────────────────────────────
    auto header = juce::Rectangle<int> (0, 0, kW, 50);
    ...
    synthTabBtn.setColour (juce::TextButton::buttonColourId,
        currentTab == 0 ? InjuryLAF::kAccent2 : InjuryLAF::kPanel);
    fxTabBtn.setColour (juce::TextButton::buttonColourId,
        currentTab == 1 ? InjuryLAF::kAccent2 : InjuryLAF::kPanel);
    ...
    for (auto& pnl : oscPanels)
    {
        ...
        pnl->cancelBtn.setVisible   (showSynth && analysisOverlay[&pnl - &oscPanels[0]]);
    }
...
//==============================================================================
void InjuryEditor::layoutSynthTab()
{
    // ── Oscillator area ────────────────────────────────────────────────────────
...
//==============================================================================
void InjuryEditor::openFileForOsc (int oscIndex)
{
            ...
            if (f.existsAsFile())
            {
                if (!proc.loadSampleFile (activeFileOsc, f))
                    juce::AlertWindow::showMessageBoxAsync (
                        juce::AlertWindow::WarningIcon, "Injury",
                        "Could not open: " + f.getFileName());
            }
        });
}

void InjuryEditor::buttonClicked (juce::Button* b)
{
    if (b == &synthTabBtn)
    {
        currentTab = 0;
...
//==============================================================================
void InjuryEditor::analysisStarted (int oscIndex)
{
...
void InjuryEditor::analysisFinished (int oscIndex)
{
...
void InjuryEditor::analysisCancelled (int oscIndex)
{
    analysisOverlay[oscIndex] = false;
    if (currentTab == 0)
        oscPanels[(size_t)oscIndex]->cancelBtn.setVisible (false);
...
void InjuryEditor::sampleLoaded (int oscIndex)
{
    if (currentTab == 0)
